...
class _Preference extends State<Preference> {
  ...
  loadData() async {
    ...
    //Load Segments selected from another collection
    ...
    _loadedRegisteredSegments.add('Sin segmentación');

    fUser.docs.forEach((element) {
      _loadedRegisteredSegments.add(element.id);
    });
    ...
    q.docs.forEach((data) {
      if (data.exists) {
        //segments inside document
        List<dynamic> segmentsList = data.data()['preferences']['segments'];

        if (segmentsList != null) {
          segmentsList.forEach((element) {
            _segmentsSelected.add(element);
          });
        }
        ...
        //Genders
        List<dynamic> _gendersList = data.data()['preferences']['genders'];
        if (_gendersList != null) {
          _gendersList.forEach((element) {
            _multipleSelected.add(element);
          });
        }
        ...
        var _goodListener = data.data()['badgets']['good_listener'];
        var _goodTalker = data.data()['badgets']['good_talker'];
        if (_funny == 1) insignias[0] = 1;
        if (_goodListener == 1) insignias[1] = 1;
        if (_goodTalker == 1) insignias[2] = 1;
      }
    });
    callState();
  }
  ...
  insertData() async {
    ...
    _segmentsSelected.removeWhere((String name) => name == 'Sin segmentación');
  ...
  Iterable<Widget> get segmentsMultipleChip sync* {
    for (String segment in _loadedRegisteredSegments) {
      yield Padding(
        ...
        child: FilterChip(
            ...
            onSelected: (bool selected) {
              setState(() {
                if (selected) {
                  if (segment.toString() == 'Sin segmentación') {
                    _segmentsSelected = [];
                  } else {
                    _segmentsSelected.removeWhere(
                        (String name) => name == 'Sin segmentación');
                  }
